...
public class SelectCitiesDialogActivity extends Activity {
	...
	private void initView() {
		...
		if (mAddress != null && !mAddress.equals("")) {
			mAddressList = mAddress.split(separator);
		}
		...
		for (int i = 0; i < mProvinceDatas.length; i++) {
			...
			if (mAddress != null && !mAddress.equals("")
					...
					&& mAddressList[0].equals(mProvinceDatas[i])) {
				selectPro = i;
			}
		...
		mAreaAdapter = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item);
		mAreaAdapter
				.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
	...
	private void setupViewsListener() {
		mSpProvince.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				if (isFirstLord) {
					if (mAddress != null && !mAddress.equals("")
							&& mAddressList.length > 1
							&& mAddressList.length < 3) {
						updateCitiesAndAreas(mCurrentProviceName,
								mAddressList[1], null);
					} else if (mAddress != null && !mAddress.equals("")
							&& mAddressList.length >= 3) {
						updateCitiesAndAreas(mCurrentProviceName,
								mAddressList[1], mAddressList[2]);
					} else {
	...
	private void updateAreas(Object object, Object myArea) {
		...
		if (!isAreas && ifSetFirstAddress && mAddress != null
				&& !mAddress.equals("") && mAddressList.length == 3) {
	...
	public void showChoose(View view) {
		...
		mCurrentAreaName = mSpArea.getSelectedItem() == null ? "" : mSpArea
				.getSelectedItem().toString() + " ";
		intent.putExtra("address", mSpProvince.getSelectedItem() + separator
				+ mSpCity.getSelectedItem() + separator + mCurrentAreaName
				+ separator + mEtDetailAddre.getText().toString());
		setResult(SELECT_CITY, intent);
		finish();
	}

	/**
	 * 解析整个Json对象，完成后释放Json对象的内存8
	 */
